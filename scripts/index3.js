...
let layer2 = document.querySelector(".layer2-hidden")
let screens =document.querySelector(".screens-wrapper")
...
//GET AVAILABLE SCREENS ON LOAD
window.addEventListener("load",async()=>{
   let sources =await window.api.getSources()
   if(sources.length ==1 ){
    document.querySelector("h2").innerText="there are no other screens opened record the entire screen instead..."
   }
 sources.map((data,index)=>{
    ...
    //let src =sources[index].thumbnail.toDataURL()
...
let chooseScreen =async(e)=>{
    screen_id=e.target.id
    layer1.classList.replace("layer1","hide")
layer2.classList.replace("layer2-hidden","layer2")
document.body.style.overflow='hidden'
let videoStream = await navigator.mediaDevices.getUserMedia({audio:false,video:{
    mandatory:{
